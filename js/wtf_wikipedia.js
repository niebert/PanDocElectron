...
var from_api = function from_api(page_identifier, lang_or_wikiid, cb) {
  if (typeof lang_or_wikiid === 'function') {
    // 2 Parameters ONLY - second is CallBack
    //----------------------------------------
    // language ID is missing - second parameter is the callback function "cb"
    cb = lang_or_wikiid; // set callback function to second parameter i.e. here lang_or_wikiid
    lang_or_wikiid = 'en'; // set default language "en" because it was defined by a function before
  };
  // 1 Parameters ONLY - Check
  // -------------------------
  // if even the second language paramter is missing then define a "noop" function as default
  cb = cb || function () {};
  lang_or_wikiid = lang_or_wikiid || 'en'; // if lang
  if (!fetch) {
    //no http method, on the client side
    console.log("Now Callback function defined in from_api");
    return cb(null);
  };
